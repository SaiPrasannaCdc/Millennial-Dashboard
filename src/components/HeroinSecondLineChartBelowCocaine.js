...
const HeroinSecondLineChartBelowCocaine = ({ region, width, height }) => {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
            fetch(process.env.PUBLIC_URL + '/data/Millenial-Format.normalized.json')
              ...
              const cData = UtilityFunctions.getGroupedData(data, rgn, 'Cocaine', 'CoPositive', 'Quarterly', ['Opioids']);
              const mData = UtilityFunctions.getGroupedData(data, rgn, 'Methamphetamine', 'Positivity', 'Quarterly', ['Methamphetamine']);
              const hData = UtilityFunctions.getGroupedData(data, rgn, 'Heroin', 'Positivity', 'Quarterly', ['Heroin']);
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', marginBottom: 40 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', margin: '24px 0 12px 0', gap: 24, flexWrap: 'wrap' }}>
        ...
        {UtilityFunctions.getToggleControls('HeroinSecondLineChartBelowCocaineToggle', setShowPercentChange, setShowLabels, showPercentChange, showLabels)} 
